...
@export var building_scene: PackedScene = preload("res://scenes/buildings/ProjectileBuilding.tscn")

@export var raycast_length: float = 20000.0

# Placement area (node-based)
@export var use_placeable_root: bool = false
@export_node_path("Node") var placeable_root_path: NodePath
@export var allow_descendants_of_placeable: bool = true

# Other validators (optional)
@export var accept_plane_mesh: bool = true
@export var accept_horizontal_surfaces: bool = true
@export var min_up_dot: float = 0.2
...
func _make_building_instance(is_ghost: bool) -> Node3D:
	...
	if inst_node == null or not (inst_node is Node3D):
		return null
