class_name ProjectilePopupUI extends BuildingPopupUI
...
@onready var projectile_speed_label := $PanelContainer/VBoxContainer/ProjectileSpeedHBox/StatValueLabel
...
func set_popup_ui_stats(stats: Dictionary) -> void:
	...
	projectile_speed_label.text = str(stats.get("projectile_speed", 0))
