...
func _process(delta: float) -> void:
	...
	query.collide_with_areas = false  # reduce flicker if you had Areas above ground

	# EXCLUDE the ghost from the ray so it never hits itself
	...
	if rayResult.size() > 0:
		# Always resolve the cell from the GridMap, regardless of which collider we hit
		...
		# Only placeable on grass tiles (id 109)
		if gridmap.get_cell_item(cell) == 109:
			...
			# DON'T immediately hide; keep last valid so it doesn't flicker
			if _has_valid:
				ghostObject.visible = true
		...
		# No hit this frame: keep showing last valid if we have one
		if _has_valid:
			ghostObject.visible = true
			ghostObject.global_position = _last_valid_world_pos
		else:
...
# Collect all CollisionObject3D RIDs under 'n' so we can exclude them in the ray
